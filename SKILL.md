...
只需在终端运行以下一行组合命令，即可直接将默认的服装操作手册转化为**格式精美的双语手册**：

```bash
# 1. 进入技能文件夹
cd /Users/shenweitao/Desktop/docx_translator_skill

# 2. 一键执行合并并注入双语文本 (默认生成印尼服装SRM操作手册_双语.docx)
...
│   ├── trans_result_part1.json       # 翻译第一分卷 (0-170)
│   └── trans_result_part2.json       # 翻译第二分卷 (171-347)
...
| 命令行参数 | 默认值 | 作用与示例 |
| :--- | :--- | :--- |
| `-i`, `--input` | `...操作手册.docx` | 输入的原始 DOCX 文件路径 |
| `-t`, `--translation` | `examples/sample_result.json` | 翻译好的 JSON 映射路径 |
| `-o`, `--output` | `...操作手册_双语.docx` | 输出的双语对照版 DOCX 路径 |
